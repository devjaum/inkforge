...
export interface LoreEntityData {
  name: string
  type: string
  typeLabel: string
  summary: string
...
function loreAppendixHtml(lore: LoreEntityData[]): string {
  if (!lore.length) return ''

  // Group by typeLabel
  const groups = new Map<string, LoreEntityData[]>()
  for (const e of lore) {
...
function bookHtml(book: BookData): string {
...
export async function exportPdf(book: BookData): Promise<{ ok: boolean; error?: string }> {
  ...
  const tmpHtml = path.join(os.tmpdir(), `inkforge-${Date.now()}.html`)
  fs.writeFileSync(tmpHtml, bookHtml(book), 'utf-8')
...
export async function exportEpub(book: BookData): Promise<{ ok: boolean; error?: string }> {
  const { canceled, filePath } = await dialog.showSaveDialog({
    title: 'Exportar EPUB',
...
`)
    ...
    const xhtmlWrap = (id: string, title: string, bodyHtml: string) => `<?xml version="1.0" encoding="utf-8"?>
...
<body>${bodyHtml}</body>
    ...
    book.chapters.forEach((ch, i) => {
      chapFolder.file(`${chapterIds[i]}.xhtml`, xhtmlWrap(
        chapterIds[i], ch.title,
        `<h1>${escapeHtml(ch.title)}</h1>${ch.htmlContent}`
      ))
    })
...
<div class="lore-group">
  ...
  <div class="lore-entry">
    ...
    ${e.tags.length ? `<p class="lore-tags">${e.tags.map(t => escapeHtml(t)).join(' · ')}</p>` : ''}
      ...
      chapFolder.file('appendix.xhtml', xhtmlWrap(
        'appendix', 'Apêndice — Guia do Universo',
        `<h1>Apêndice — Guia do Universo</h1>
...
${loreSections}`
    ...
    // content.opf
    const manifestItems = allIds.map((id, i) =>
      `<item id="${id}" href="${allFiles[i]}" media-type="application/xhtml+xml"/>`
    ).join('\n    ')
...
async function findEbookConvert(): Promise<string | null> {
  const candidates = [
    ...
    path.join(os.homedir(), 'AppData', 'Local', 'Programs', 'Calibre2', 'ebook-convert.exe'),
...
export async function exportMobi(book: BookData): Promise<{ ok: boolean; error?: string; needsCalibre?: boolean }> {
  ...
  const tmpDir  = path.join(os.tmpdir(), `inkforge-${Date.now()}`)
...
<ncx xmlns="http://www.daisy.org/z3986/2005/ncx/" version="2005-1">
  ...
  <navMap>${book.chapters.map((ch, i) => `
    ...
    fs.writeFileSync(tmpEpub, buf)

    await execAsync(`"${converter}" "${tmpEpub}" "${tmpMobi}"`)

    const mobiData = fs.readFileSync(tmpMobi)
    fs.writeFileSync(filePath, mobiData)
    return { ok: true }
  } catch (e) {
    return { ok: false, error: String(e) }
  } finally {
    try { fs.unlinkSync(tmpEpub) } catch {}
    try { fs.unlinkSync(tmpMobi) } catch {}
  }
}
